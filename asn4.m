...
define(FLOAT_MEM, 4)                        ! 4 bytes for memory allocation of float

define(float1_r, l0)                        ! holds first float from user
define(float2_r, l1)                        ! holds 2nd float from user
...
main:
    ...
    mov     %o0, %float1_r                  ! store 1st float so it can be sent to FPU
    ...
    mov     %o0, %float2_r                  ! store 2nd float so it can be sent to FPU
    
    st      %float1_r, [%fp + FLOAT_MEM]    ! save 1st float into memory
    ld      [%fp + FLOAT_MEM], %f0          ! load 1st float from memory into FPU
    
    st      %float2_r, [%fp + FLOAT_MEM]    ! save 2nd float into memory
    ld      [%fp + FLOAT_MEM], %f1          ! load 2nd float from memory into FPU
    
    fadds   %f0, %f1, %f0                   ! add the 2 floating point values
    
    st      %f0, [%fp + FLOAT_MEM]          ! store result in memory
    ld      [%fp + FLOAT_MEM], %o0          ! load result into output register
    ...
    mov     %float1_r, %o0                  ! pass floating values to myfadd function
    call    myfadd
    mov     %float2_r, %o1
